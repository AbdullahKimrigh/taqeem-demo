...
@section('title', __('Submit Accident'))
...
<div class="max-w-2xl mx-auto">
    ...
    <form action="{{ route('customer.store') }}" method="POST" enctype="multipart/form-data" class="bg-white rounded-lg border border-slate-200 p-6 shadow-sm">
        ...
        <div class="mb-4">
            <label for="accident_number" class="block text-sm font-medium text-slate-700 mb-1">{{ __('Accident Number *') }}</label>
        ...
        <div class="mb-4">
            <label for="plate_number" class="block text-sm font-medium text-slate-700 mb-1">{{ __('Plate Number *') }}</label>
        ...
        <div class="mb-6">
            <label class="block text-sm font-medium text-slate-700 mb-1">{{ __('Vehicle Images (min. 6: 4 sides + damage area) *') }}</label>
            ...
            @error('images')
                <p class="text-red-600 text-sm mt-1">{{ __($message) }}</p>
            @enderror
            @error('images.*')
                <p class="text-red-600 text-sm mt-1">{{ $message }}</p>
            @enderror
